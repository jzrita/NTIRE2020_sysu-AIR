...
class Discriminator_VGG_128(nn.Module):
    def __init__(self, in_nc, base_nf, norm_type='batch', act_type='leakyrelu', mode='CNA', HR_size=128):
        ...
        conv3 = B.conv_block(base_nf*2, base_nf*2, kernel_size=4, stride=2, norm_type=norm_type, \
            act_type=act_type, mode=mode)
        SA1 = Self_Attn(128, nn.LeakyReLU(0.2, inplace=True))
        # HR_size/4, 128
        ...
        conv7 = B.conv_block(base_nf*8, base_nf*8, kernel_size=4, stride=2, norm_type=norm_type, \
            act_type=act_type, mode=mode)
        SA2 = Self_Attn(512, nn.LeakyReLU(0.2, inplace=True))
        # HR_size/16, 512
        ...
        conv9 = B.conv_block(base_nf*8, base_nf*8, kernel_size=4, stride=2, norm_type=norm_type, \
            act_type=act_type, mode=mode)
        # HR_size/32, 512
        self.features = B.sequential(conv0, conv1, conv2, conv3, SA1,\
             conv4, conv5, conv6, conv7, SA2, conv8, conv9)

        # classifier
        self.classifier = nn.Sequential(
            nn.Linear(512 * (int(HR_size/32)) * (int(HR_size/32)), 100), nn.LeakyReLU(0.2, True), nn.Linear(100, 1))
