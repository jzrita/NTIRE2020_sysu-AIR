...
from networks import create_model, define_D
from .base_solver import BaseSolver
from networks import init_weights
from utils import util

from .myloss_func import myloss_func, GANLoss
...
class SRSolver(BaseSolver):
    ...
    def train_step(self, step):
        # only support split_batch 1
        if self.split_batch != 1:
            raise ValueError("[Error] The 'split_batch' should be 1")
        if self.D_update_ratio != 1 or self.D_init_iters !=0:
            raise ValueError("[Error] The 'D_update_ratio' should be 1 and 'D_init_iters' should be 0")
        if not self.use_cl:
            raise ValueError("[Error] The 'use_cl' should be True ")
        ...
        l_g, loss_log = self.loss_func.compute_loss(outputs, self.HR, self.discriminator) 
        l_g.backward()
        self.optimizer.step()
        ...
        l_d = self.loss_func.compute_D_loss(outputs, self.HR, self.discriminator)
        loss_log['discriminator_loss'] = l_d
        l_d.backward()
        self.optimizer_D.step()

        return (l_d + l_g).item() , loss_log
